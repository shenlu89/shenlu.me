...
Given this minimal data size, there’s really no need to build a full backend or database. Instead, I chose to store all data—including user settings and writing history—directly in the browser using **localStorage**. This approach ensures that the app is fast, simple, and completely private, while still preserving user data across sessions.
