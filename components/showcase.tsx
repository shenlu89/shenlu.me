...
const Showcase = ({ href, title, logoUrl, imageUrl, description }: any) => {
  return (
    <Link
      href={href}
      target="_blank"
      className="flex flex-start justify-between w-full space-x-4 no-underline p-6 border bg-slate-50 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-black dark:border-slate-600 border-slate-200 rounded"
    >
      <div className="flex item-center w-full md:flex-row justify-between space-y-4">
        <div className="flex flex-col w-full items-center md:items-start">
          ...
          <p className="flex w-full text-start text-slate-600 dark:text-slate-200">
            {description}
          </p>
        </div>
      </div>
